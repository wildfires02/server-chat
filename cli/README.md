...
`cli` 是基于 **Go 语言** 原生实现的 IM 交互式及批处理命令行客户端。它使用高效率的 [gRPC API](../pbx/) 直接与 IM 服务端通信，支持互动对话、实时订阅、状态变更以及全套服务器管理控制指令。
...
```bash
./cli -host=localhost:16060 -login-basic=alice:alice123 -verbose
```
...
在交互模式或批处理脚本中，`cli` 提供了强大的变量与结果引用能力：
...
```bash
./cli --no-login < sample-script.txt
```
