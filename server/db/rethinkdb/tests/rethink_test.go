...
import (
	...
	backend "chat/server/db/rethinkdb"
	"chat/server/logs"
	"chat/server/store"
...
func TestMessageAttachments(t *testing.T) {
	...
	// 检查是否 attachments were linked to 消息
	...
	// 检查是否 use count was incremented in fileuploads
...
func TestMessageDeleteList(t *testing.T) {
	...
	// Check 消息 in Topic - some should be soft deleted
	...
	// 检查是否 消息 content was cleared (hard delete)
