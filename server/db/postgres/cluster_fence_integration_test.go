...
// TestClusterFencingIntegration 使用真实 PostgreSQL 验证旧 Owner 无法越过数据库 fence。
// 默认跳过；设置 IM_TEST_POSTGRES_FENCE_DSN 后会重建 DSN 指向的测试数据库。
func TestClusterFencingIntegration(t *testing.T) {
	dsn := os.Getenv("IM_TEST_POSTGRES_FENCE_DSN")
	if dsn == "" {
		t.Skip("未设置 IM_TEST_POSTGRES_FENCE_DSN")
	}
	...
	if _, err = database.db.Exec(
		context.Background(),
		"INSERT INTO users(id,createdat,updatedat,state) VALUES($1,$2,$3,$4)",
...
func verifyClusterFenceLifecycle(t *testing.T, database *adapter, now time.Time) {
	...
	if err := database.MessageSaveAtomic(fencedPostgresMessage(now, 1, clusterID, 10, "im-0")); err != nil {
		t.Fatalf("首次 Owner 写入失败：%v", err)
	}
	if err := database.ClusterFenceAdvance(clusterID, 11); err != nil {
		t.Fatal(err)
	}
	if err := database.MessageSaveAtomic(fencedPostgresMessage(now, 2, clusterID, 10, "im-0")); !errors.Is(err, types.ErrClusterFenced) {
		t.Fatalf("旧 Owner 写入错误 = %v，期望 %v", err, types.ErrClusterFenced)
	}
	if err := database.MessageSaveAtomic(fencedPostgresMessage(now, 2, clusterID, 11, "im-1")); err != nil {
	...
	if err := database.db.QueryRow(context.Background(),
		"SELECT clusterowner,clusterepoch,seqid FROM topics WHERE name=$1", "sys").
...
// fencedPostgresMessage 创建只用于 PostgreSQL 集成测试的集群消息。
func fencedPostgresMessage(
	now time.Time,
	sequence int,
	clusterID string,
	epoch int64,
