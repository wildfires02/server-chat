...
- “群组语音/视频通话使用 Agora”已经完成服务端 AccessToken2、群 ACL 角色、加入/离开/续期和状态持久化；集成工程的 Web 客户端和 Flutter App 已接入 Agora，真实项目跨区域联调仍待完成。
...
这些能力已经形成协议闭环。联系人使用持久缓存 CAS、双边事务日志和游标分页；文件
处理使用持久任务、跨节点租约和共享断点续传。生产集群仍需目标硬件容量和故障演练，
不应把“代码闭环”直接等同于“生产容量已验收”。
...
| 社区治理 | ACL、禁言、封禁、Slow Mode、举报队列、审核后台和审计 | 申诉、自动内容审核和成熟反垃圾模型 |
...
| Forum Topics | 未实现；邀请链接、入群审批和 Slow Mode 已实现 |
| 频道浏览量、转发量、评论区、讨论组和数据分析 | 未实现 |
| 成熟反垃圾与申诉 | 未实现；举报、审核队列与管理后台已实现 |
...
| 🟡 | 客户端与用户体验 | server-chat 仓库不含客户端；Groupbuying 已集成 ecommerce Web 与 Flutter mall_app，桌面端和完整端到端验收未完成 | 55%–65% |
...
- 当前数据库版本为 `123`；`119→120` 为 Topic 增加带字段注释的
  `clusterowner`、`clusterepoch`，`120→121` 为官方大群成员游标查询补齐复合
  索引，`121→122` 为群消息 Seen by 增加订阅阅读时间检查点，`122→123` 为
  90 天消息保留增加分批清理索引。完整记录见
...
- 外部 GIF 搜索、热门推荐和结果排序。
- 按接收者投影的中英双向自动消息翻译。
- 可配置的机构客户字段模板、批量导入和 CRM 自动化规则（基础资料、备注和三级置顶已实现）。
- 语音转文字。
- 编辑历史与历史版本审计。
...
社区治理已经实现多种托管邀请（过期、限次数、撤销）、`open/invite/approval/closed`
入群策略、审批队列、Slow Mode、消息举报审核和管理审计。当前仍未实现：
...
- 主动说话人检测。
- 分区域媒体节点。
- 集中式音视频质量告警与长期报表（Web 与 Flutter 已采集网络质量，Flutter 已启用双码流和弱网 ABR）。
...
联系人和分组使用持久缓存 CAS 与版本号处理多节点并发，双边好友变更写入可恢复事务
日志，并具备稳定分页、拒绝/拉黑状态和频率/总量配额。以下能力仍未实现：
...
文件处理已经使用持久任务、Worker 租约、重试/退避/死信和指标；共享媒体分块支持
跨节点续传和重启恢复，S3 使用浏览器 Multipart 直传。恶意文件物理隔离并支持人工
释放/删除；图片、视频和音频生成多规格变体，相同上传者的安全内容按摘要复用派生
资源。以下生产能力仍未实现：

- 自研共享分块路径的每块 SHA-256 协商。
- 消息硬删除、附件替换或 Topic 删除后的文件 ACL 引用回收。
- 对外部 HTTPS 文件的服务端 ACL、安全扫描和生命周期管理；当前只管理本地媒体 URL。
- 转码 CPU/磁盘资源隔离和真实 ClamAV/FFmpeg/LibreOffice 集成环境的故障/容量测试。
...
11. ❌ 处理许可证和上游归属问题。
12. ❌ 明确首个正式客户端方案。
13. ✅ 文件处理已迁移到持久任务队列，并实现跨节点共享分块和 S3 Multipart 断点续传。
14. 🟡 联系人已补齐多节点 CAS 与好友双方事务日志；迁移到正式关系表仍是后续容量优化。
15. 🟡 已完成通知偏好、可靠推送队列、重试、DLQ、管理页和 Webhook；供应商送达 SLO 待生产验收。
...
3. ✅ 群组托管邀请、撤销、使用次数和入群审批。
4. 🟡 举报、审核队列和管理审计已完成；自动内容审核与申诉未实现。
5. ❌ 频道浏览量、转发量和评论区。
6. ❌ 用户素材包安装/收藏/最近使用，以及外部 GIF 搜索、热门排序和内容治理。
7. ✅ Groupbuying 集成工程已有 ecommerce Web 和 Flutter mall_app；server-chat 独立仓库不携带客户端源码。
8. ❌ 端到端自动化测试。
9. ✅ 官方只读频道、机构身份联调及客户归属私聊隔离。
10. 🟡 中英 P2P 纯文本自动翻译与客户资料、备注、三级置顶已实现；供应商额度、机构词库和失败告警待生产配置。
...
> 一个具备多数据库、集群、富文本与媒体消息、角色化群组、官方群治理、服务端强制只读广播频道、权限感知搜索、可靠文件/Push 管道和 Agora 跨端通话能力的 Tinode 风格 IM 底座。
...
1. 先完成安全、稳定的私有化 IM。
2. 在已完成基础消息、跨端客户端、群组治理和当前会话搜索的前提下，补齐全局搜索、申诉、自动审核和频道运营能力。
