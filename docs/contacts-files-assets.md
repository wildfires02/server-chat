...
本页前述接口已经可用。以下仅列仍需生产验收或后续增强的边界。

### 联系人

- 联系人状态使用持久缓存 CAS 和版本号处理多节点并发写入；好友双方变更写入事务
  日志，失败可恢复，不再依赖单进程互斥锁。
- 好友申请、接受、拒绝、解除关系和拉黑均有双边状态约束，并提供申请频率、每日新增、
  联系人总量和批量变更配额。生产环境仍需按实际用户规模调整阈值并压测热点账号。
- 大通讯录采用稳定游标分页，不再把完整目录作为一次协议响应；极大规模场景仍建议
  迁移到独立关系表和索引服务以降低持久缓存热点。
- 手机号/邮箱哈希上传、隐私授权和通讯录匹配仍未实现；拒绝、拉黑、限流和基础
  反骚扰已经完成。
...
- 文件处理现已使用持久任务、Worker 租约、自动重试、指数退避、死信和 expvar 指标；
  完成状态仍以文件处理元数据为业务事实。
- 断点续传支持完整 tus 核心头、共享媒体分块、跨节点续传和重启恢复；S3 生产配置
  还支持浏览器预签名 Multipart Upload。自研共享分块路径尚未增加每个分块的独立摘要协商。
- 文件 ACL 当前以追加 Topic/用户授权为主，消息硬删除、附件替换或 Topic 删除后尚未
  回收对应授权。
- 外部 HTTPS 文件不由本服务的 ACL、扫描和生命周期管理；如产品允许外链，需要新增
  代理下载、URL 信誉检测和隔离扫描。
- 命中恶意文件后会移动到物理隔离区；管理员可在审核页查看、带原因释放或永久删除，
  所有动作写入审计。文件元数据记录 ClamAV 版本，启动瞬时不可用时会定期重试查询。
- 图片生成 320/1024/2048 WebP，视频生成海报及 360p/720p/1080p H.264 档位，音频
  生成 Opus；客户端按网络类型选择档位。同一上传者、相同 MIME 和 SHA-256 的安全文件
  复用派生资源，但 original 地址始终保持独立，禁止跨用户去重泄漏。
- ClamAV、FFmpeg、LibreOffice 的真实组合故障、CPU/磁盘隔离和目标硬件容量仍需生产压测。
...
- 群组通话的服务端控制面使用 Agora，项目自身没有 SFU/MCU；屏幕共享、录制、直播、
  ...
  `agora_rtc_engine` 双码流、弱网 ABR 和质量回传，真实蜂窝网络与跨区域质量仍需验收。
- 官方群已完成可撤销/限次数邀请、入群审批、Slow Mode、举报审核和管理审计；
  Forum Topics、申诉流程和自动内容审核仍未实现。
