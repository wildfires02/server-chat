...
`keygen` 是用于为 [IM 服务端](../../internal/server/) 生成与校验客户端 API Key 的命令行工具。
...
* **`HMAC Salt`（私钥盐值）**：配置在服务端 `im.yaml` 配置文件中校验客户端 Key。
...
1. **服务端**：将 `HMAC salt` 的值复制粘贴到 IM 服务端配置文件 `im.yaml` 的 `api_key_salt` 参数项中。
