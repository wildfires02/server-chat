# IM REST 外部认证服务器示例 (Go 实现)

本模块是基于 **Go 语言** 原生实现的 IM 外部 [REST 认证适配服务](../server/auth/rest/)。
...
```bash
go run main.go -port=8080 -data=dummy_data.json
```
...
若要让 IM 主服务端启用本 REST 认证服务，需修改服务端配置文件 `im.conf` 中的 `auth_config` 配置节：

```json
"auth_config": {
  "logical_names": ["basic", "token"],
  "adapters": {
    "rest": {
