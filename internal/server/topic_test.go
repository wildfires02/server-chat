...
func TestHandleBroadcastAgoraGroupCall(t *testing.T) {
	...
	if party.agora == nil ||
		party.agora.role != agoraRolePublisher ||
		party.agora.uid == 0 {
	...
	if credentials.Provider != constCallProviderAgora ||
		...
		credentials.Channel != helper.topic.currentCall.agora.channel ||
		credentials.UID != party.agora.uid ||
		credentials.Token == "" ||
		credentials.Role != agoraRolePublisher ||
