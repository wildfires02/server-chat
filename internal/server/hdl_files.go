...
func largeFileReceiveHTTP(wrt http.ResponseWriter, req *http.Request) {
	...
	reader, mimeType, err := prepareFileUploadReader(file, header.Header.Get("Content-Type"))
	if err != nil {
	...
	url, fdef, _, err := uploadAndFinalizeFile(
		mh,
		store.Files,
		fdef,
		reader,
		globals.maxFileUploadSize,
	)
	if err != nil {
		logs.Info.Println("media upload: failed", err)
	...
	logs.Info.Println("media upload: ok")
}
...
func (*grpcNodeServer) LargeFileReceive(stream pbx.Node_LargeFileReceiveServer) error {
	...
	uploadReader := &grpcFileUploadReader{
		receiver: stream,
		pending:  req.GetContent(),
	}
	reader, mimeType, err := prepareFileUploadReader(uploadReader, req.GetMeta().GetMimeType())
	if err != nil {
		writeResponse(ErrMalformed(msgID, "", now), err)
		return nil
	}
	...
	url, fdef, size, err := uploadAndFinalizeFile(
		mh,
		store.Files,
		fdef,
		reader,
	...
	if err != nil {
		logs.Info.Println("media upload: failed", err)
		if errors.Is(err, errFileUploadTooLarge) {
			writeResponse(ErrTooLarge(msgID, "", now), err)
		} else {
			writeResponse(decodeStoreError(err, msgID, now, nil), err)
		}
		return nil
	...
	if err != nil {
		logs.Info.Println("media upload: response failed", err)
	} else {
		logs.Info.Println("media upload: ok")
	}
	return err
...
func authFileRequest(authMethod, secret, sid, remoteAddr string) (types.Uid, []byte, error) {
	...
	if authMethod != "" {
		...
		if err != nil {
			logs.Info.Println("media: invalid auth secret encoding", authMethod)
