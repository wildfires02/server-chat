...
do
  ...
  # 基于本地源码构建镜像
  docker ${buildcmd} -f deployments/docker/im/Dockerfile --build-arg VERSION=$tag --build-arg TARGET_DB=${dbtag} ${buildtags} .
done
...
docker rmi ${rmitags} 2>/dev/null || true
docker ${buildcmd} -f deployments/docker/chatbot/Dockerfile --build-arg VERSION=$tag ${buildtags} .
...
docker rmi ${rmitags} 2>/dev/null || true
docker ${buildcmd} -f deployments/docker/exporter/Dockerfile --build-arg VERSION=$tag ${buildtags} .
