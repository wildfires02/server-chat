...
USAGE="Usage: $0 [ --config <path_to_im.yaml> ] {start|stop}"

# 服务器二进制文件可能具有不同的名称和路径。
SERVER="$REPO_ROOT/bin/im-server"
  ...
  case "$key" in
    ...
    start)
      if [ ! -z "$config" ] ; then
        ...
        IM_CONF="$REPO_ROOT/configs/im.yaml"
      fi
      if [ ! -z "${static_data+x}" ] ; then
        STATIC_DATA_DIR=$static_data
      else
        STATIC_DATA_DIR="$REPO_ROOT/web/static"
      fi
      ...
      do
        # 显式覆盖为开发集群模式，再启动当前节点。
        IM_RUNTIME__DEPLOYMENT_MODE=cluster $SERVER -config=${IM_CONF} -cluster_self=${NODE_NAME} -listen=:${HTTP_PORT} -grpc_listen=:${GRPC_PORT} -static_data=${STATIC_DATA_DIR} -log_flags=stdFlags,shortfile &
        # 将节点 PID 保存到临时文件。
