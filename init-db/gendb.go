...
func genDb(data *Data, p2pDel bool) {
	...
	for _, uu := range data.Users {
		...
		// 存储.用户.Create 会将用户订阅至 !me Topic，但不会自动创建 !me Topic
		if _, err := store.Users.Create(&user, uu.Private); err != nil {
			log.Fatal(err)
		}

		// 保存凭据：保存电子邮件和电话号码（视为已验证状态）。
		if uu.Email != "" {
	...
	if messageCount > 0 {
		...
		if messageCount > 1 {
			...
			for i := range toInsert {
				// At least 20% of subsequent 消息 should come from the same 用户 in the same Topic.
				if rand.Intn(5) > 0 {
					subIdx = rand.Intn(len(data.Groupsubs) + len(data.P2psubs)*2)
				}
				...
				if subIdx < len(data.Groupsubs) {
					if data.Groupsubs[subIdx].AsChan {
						// Channel readers should not have any published 消息.
						continue
					}
				...
				// Max time between 消息 is 2 hours, averate - 1 hour, time is increasing as seqId increases
...
func parsePublic(public *theCard, path string) *card {
	...
	if fname := public.Photo; fname != "" {
		...
		if strings.HasPrefix(fname, "http://") || strings.HasPrefix(fname, "https://") {
			...
			if err != nil {
				log.Fatalf("Failed to fetch image from %s: %v", fname, err)
			}
			defer resp.Body.Close()
			photo.Data, err = io.ReadAll(resp.Body)
			if err != nil {
				log.Fatalf("Failed to read image data from %s: %v", fname, err)
			}
		} else {
