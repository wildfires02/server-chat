...
# 复制服务器配置文件 im.conf、数据库初始化脚本及示例数据
cp ./server/im.conf ./releases/tmp/
cp ./init-db/data.json ./releases/tmp
cp ./init-db/credentials.sh ./releases/tmp
...
then
  ...
  cp ./server/static/img/*.png ./releases/tmp/static/img 2>/dev/null || true
  cp ./server/static/img/*.svg ./releases/tmp/static/img 2>/dev/null || true
  cp ./server/static/img/*.jpeg ./releases/tmp/static/img 2>/dev/null || true
  cp ./server/static/img/bkg/*.png ./releases/tmp/static/img/bkg 2>/dev/null || true
  cp ./server/static/img/bkg/*.jpg ./releases/tmp/static/img/bkg 2>/dev/null || true
  cp ./server/static/img/bkg/*.json ./releases/tmp/static/img/bkg 2>/dev/null || true
  cp ./server/static/audio/*.m4a ./releases/tmp/static/audio 2>/dev/null || true
  cp ./server/static/css/*.css ./releases/tmp/static/css 2>/dev/null || true
  cp ./server/static/index.html ./releases/tmp/static 2>/dev/null || true
  cp ./server/static/index-dev.html ./releases/tmp/static 2>/dev/null || true
  cp ./server/static/version.js ./releases/tmp/static 2>/dev/null || true
  cp ./server/static/umd/*.js ./releases/tmp/static/umd 2>/dev/null || true
  cp ./server/static/manifest.json ./releases/tmp/static 2>/dev/null || true
  cp ./server/static/service-worker.js ./releases/tmp/static 2>/dev/null || true
  # 生成默认空 Firebase 客户端配置文件
  echo 'const FIREBASE_INIT = {};' > ./releases/tmp/static/firebase-init.js
else
  echo "未检测到静态前端资源目录 (server/static)，跳过静态文件复制"
fi
...
do
  ...
  do
    ...
    env GOOS="${plat}" GOARCH="${arc}" go build \
      -ldflags "-s -w -X main.buildstamp=`git describe --tags 2>/dev/null || echo 'custom'`" -tags "${buildtag}" \
      -o ./releases/tmp/im${ext} ./server > /dev/null
    
    # 编译数据库初始化工具 init-db
    env GOOS="${plat}" GOARCH="${arc}" go build \
      -ldflags "-s -w" -tags "${buildtag}" -o ./releases/tmp/init-db${ext} ./init-db > /dev/null
...
cp ${GOSRC}/chat/chatbot/quotes.txt ./releases/tmp 2>/dev/null || true
...
cp ${GOSRC}/chat/cli/*.txt ./releases/tmp 2>/dev/null || true

tar -C ${GOSRC}/chat/releases/tmp -zcf ./releases/${version}/cli.tar.gz .
pushd ./releases/tmp > /dev/null
